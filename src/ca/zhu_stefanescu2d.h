...
class BlockZhuStefanescu2DManager
 ...
 public:
  ...
  /*!
   * \brief 初始化Zhu-Stefanescu 2D CA模型
   * 
   * 清除现有的BlockZS和Interfaces，然后根据BlockGeo中的块信息重新创建BlockZS对象，
   * 并初始化接口列表。每个BlockZS对象代表一个计算块，包含Zhu-Stefanescu模型的实现。
   */
  void Init() {
    BlockZS.clear();
    for (int i = 0; i < this->BlockGeo.getBlockNum(); ++i) {
  ...
  /*!
   * \brief 初始化设置Zhu-Stefanescu 2D CA模型
   * 
   * 该函数用于初始化Zhu-Stefanescu 2D CA模型，包括查找要设置的块、计算初始过冷度、
   * 设置首选生长角度等操作，并调用各个块的Setup和SimpleCapture方法。
   * 
   * \param SiteId 形核点的索引位置
   * \param num 捕获的邻居数，默认为CALatSet::q
   */
   ...
   * \brief 获取所有块的接口列表
   * 
   * 返回一个指向每个BlockZS对象接口向量的指针向量引用
   * 
   * \return std::vector<std::vector<std::size_t>*>& 所有块的接口列表引用
   */
   ...
   * \brief 应用简单捕获过程
   * 
   * 在所有BlockZS对象上并行应用生长和简单捕获操作，并在操作之间进行通信
   */
  void Apply_SimpleCapture() {
...
#pragma omp parallel for num_threads(Thread_Num)
   ...
   * \brief 在所有相关场之间进行通信
   * 
   * 执行状态场和固相分数场的正常通信，确保在并行计算中各块之间的数据一致性
   */
  void Communicate() {
    // State field
   ...
   * \brief 获取所有接口单元的总数
   * 
   * 遍历所有块的接口列表，统计接口单元的总数量
   * 
   * \return std::size_t 接口单元总数
   */
   ...
   * \brief 获取固相晶胞总数
   * 
   * 遍历所有BlockZS对象，累加每个块中的固相晶胞数
   * 
   * \return std::size_t 固相晶胞总数
   */
   ...
   * \brief 计算固相分数
   * 
   * 根据所有BlockZS对象中的固相晶胞数和等效晶胞总数计算整体固相分数
   * 考虑了块的层级(level)对晶胞计数的影响
   * 
   * \return T 固相分数
   */
   ...
   * \brief 判断是否需要细化块单元
   * 
   * 检查块单元中是否包含固相，以决定是否需要进行细化操作。
   * 主要用于自适应网格细化(AMR)判断。
   * 
   * \param GeoHelper 几何辅助对象，包含块单元信息
   * \return bool 如果需要细化则返回true，否则返回false
   */
