...
// 相场类型定义
template <typename T>
struct PHASE_FIELD : public FieldBase<1> {
  using value_type = T;
  using array_type = GenericArray<T>;
  static constexpr unsigned int array_dim = 1;
  static constexpr bool isField = true;
  static constexpr const char* name = "PhaseField";
  
  
#ifdef __CUDACC__
  using cudev_FieldType = cudev::GenericField<cudev::GenericArray<T>, PHASE_FIELD<T>>;
#endif
};

template <typename T>
using PhaseField = GenericField<GenericArray<T>, PHASE_FIELD<T>>;

// 相场分布函数字段定义
template <typename T, unsigned int q>
struct PHASE_POP : public FieldBase<q> {
  using value_type = T;
  using array_type = StreamArray<T>;  
  static constexpr unsigned int array_dim = q;
  static constexpr bool isField = true;
  static constexpr const char* name = "PhasePopulations";
  
#ifdef __CUDACC__
  using cudev_FieldType = cudev::GenericField<cudev::StreamArray<T>, PHASE_POP<T, q>>;
#endif
};

template <typename T, unsigned int q>
using PhasePop = GenericField<StreamArray<T>, PHASE_POP<T, q>>;

// 相场梯度字段定义
template <typename T, unsigned int D>
struct GRAD_PHASE : public FieldBase<1> {
  using value_type = Vector<T, D>;
  using array_type = GenericArray<Vector<T, D>>;
  static constexpr unsigned int array_dim = 1;
  static constexpr bool isField = true;
  static constexpr const char* name = "GradPhase";
  
#ifdef __CUDACC__
  using cudev_FieldType = cudev::GenericField<cudev::GenericArray<Vector<T, D>>, GRAD_PHASE<T, D>>;
#endif
};

template <typename T, unsigned int D>
using GradPhase = GenericField<GenericArray<Vector<T, D>>, GRAD_PHASE<T, D>>;