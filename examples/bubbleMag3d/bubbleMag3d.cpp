...
// two-phase
T rho_l, rho_h, eta_l, eta_h, sigma, gravity, Eo, Re, U_g, Tau_ns, GravityScale;
...
void readParam() {
  ...
  // Note: 5x factor required to align with Guo2025 Case C (2D validated)
  H0 = std::sqrt(T(25.0)) * std::sqrt(T(2.0) * Bom * sigma / D_bubble);
  ...
  MPI_RANK(0){
    ...
    printf("Eo=%.0f Re=%.0f W=%.1f M=%.3f tau_phi=%.3f gs=%.4f\n",Eo,Re,Interface_Width,Mobility,Tau_phi,GravityScale);
...
int main(int argc, char* argv[]) {
  ...
  T xc=Bubble_Center[0]*Cell_Len, yc=Bubble_Center[1]*Cell_Len, zc=Bubble_Center[2]*Cell_Len;
  ...
  for(int b=0;b<Geo.getBlockNum();++b){
    ...
    T vs=bk.getVoxelSize();
    T mx=bk.getMin()[0],my=bk.getMin()[1],mz=bk.getMin()[2];
    int ov=0;
  ...
  // init PF pops (D3Q19 equilibrium, u=0)
  for(int b=0;b<Geo.getBlockNum();++b){
  ...
  // init MF: psi = -H0*z (uniform field, Z vertical), g_k = w_k*psi
  {
  ...
  while(t()<MaxStep){
    ...
    // 0b: Set wall psi = -H0*z  and wall pops = feq(psi_bc)
    {
      ...
      for(int b=0;b<Geo.getBlockNum();++b){
        ...
        T minZ=bk.getMin()[2],maxZ=bk.getMax()[2],vs=bk.getVoxelSize();
        if(minZ<Cell_Len*T{1.5}){
          for(int jj=0;jj<ny;++jj){
            for(int kk=0;kk<=ov;++kk){
              T z=minZ+T(kk)*vs, psi_w=-H0*z;
    ...
    // 0c: MF collision (direct iteration) — skip Z wall cells
    // Wall cells have fixed Dirichlet BC (psi=-H0*z, pops=feq) set by 0b.
    // Running collision on them deviates pops from feq; streaming then
    // propagates the deviation into the first interior layer, corrupting
    // Σg_i and the H field there.  Skipping collision keeps wall pops at
    // feq so streaming carries correct values inward.
    for(int b=0;b<Geo.getBlockNum();++b){
      auto& bl=MFLattice.getBlockLat(b); const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
      int ov=bk.getOverlap(),nz=bk.getNz();
      T minZ=bk.getMin()[2],maxZ=bk.getMax()[2];
      for(int k=ov;k<bk.getNz()-ov;++k){
        bool isZwall=false;
        if(minZ<Cell_Len*T{1.5} && k==ov) isZwall=true;
        if(maxZ>H_global-Cell_Len*T{1.5} && k==nz-ov-1) isZwall=true;
        if(isZwall) continue;
        for(int j=ov;j<bk.getNy()-ov;++j){
          for(int i=ov;i<bk.getNx()-ov;++i){
    ...
    // 0e-fix: Re-apply uniform-field psi = -H0*z for wall + ghost cells
    // (k=0..ov).  0e's Σg_i overwrites the wall cell (k=ov) psi that 0b
    // set; without restoration, MFComputeH3D computes a wrong ∇ψ at the
    // wall, amplifying Hmag near the Z boundaries.  Ghost cells (k=0..ov-1)
    // are also reset so the wall cell's downward neighbor carries the
    // correct linear profile.
    {
      auto& psiF=MFLattice.getField<PSI<T>>();
      for(int b=0;b<Geo.getBlockNum();++b){
        const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
    ...
    // 0f-fix: Re-apply uniform-field H at Z wall + ghost cells (k=0..ov).
    // Communicate() does not sync Z-direction ghost cells for wall blocks
    // (no neighbor block in Z), so ghost HMAG stays at its stale/initial
    // value (0).  This corrupts ∇|H| in MFMagneticForce3D.  Resetting the
    // wall (k=ov) and ghost (k=0..ov-1) layers to the uniform field
    // (H = H0 ẑ) restores the correct downward neighbor for the first
    // interior cell.  Note: an earlier attempt extended the reset to
    // k=ov+1 and k=ov+2, but this was counterproductive — it created a
    // steeper ∇|H| between the reset region (HMAG=H0) and the artifact
    // region (HMAG≈4×H0 at triple corners), increasing Fmag instead of
    // reducing it.  The HMAG clip (0f-clip, below) handles the artifact
    // globally without creating artificial gradients.
    {
      ...
      for(int b=0;b<Geo.getBlockNum();++b){
        const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        auto& bHx=hxF.getBlockField(b); auto& bHy=hyF.getBlockField(b);
        auto& bHz=hzF.getBlockField(b); auto& bHm=hmF.getBlockField(b);
        int nx=bk.getNx(),ny=bk.getNy(),nz=bk.getNz(),ov=bk.getOverlap();
        T minZ=bk.getMin()[2],maxZ=bk.getMax()[2];
        if(minZ<Cell_Len*T{1.5}){
          for(int kk=0;kk<=ov;++kk)
            for(int jj=0;jj<ny;++jj)
              for(int ii=0;ii<nx;++ii){
                ...
                bHx.get(id)=T{0}; bHy.get(id)=T{0};
                bHz.get(id)=H0;   bHm.get(id)=H0;
              }
        }
        if(maxZ>H_global-Cell_Len*T{1.5}){
          for(int kk=nz-ov-1;kk<nz;++kk)
        ...
        for(int k=ov;k<nz-ov;++k)
          for(int j=ov;j<ny-ov;++j)
            for(int i=ov;i<nx-ov;++i){
              ...
              T hm=bHm.get(id);
              if(hm>Hmax_clip){
                T scale=Hmax_clip/hm;
                bHx.get(id)*=scale; bHy.get(id)*=scale; bHz.get(id)*=scale;
                bHm.get(id)=Hmax_clip;
              }
            }
      }
      // Re-communicate so ghost cells of neighboring blocks reflect
      // clipped interior values before MFMagneticForce3D reads them.
      CommunicateAllMFFields3D<T>(MFLattice);
    }
    ...
    // ===== Phase D: Streaming =====
    PF_BB.Apply(t());
    { // PF Z ghost pop fix (3D: extend with jj loop over y)
      T Hg=T(Nk)*Cell_Len;
      for(int b=0;b<Geo.getBlockNum();++b){
    ...
    NS_BB.Apply(t());
    { // NS Z ghost pop fix (3D: extend with jj loop over y)
      T Hg=T(Nk)*Cell_Len;
      for(int b=0;b<Geo.getBlockNum();++b){
        auto& bl=NSLattice.getBlockLat(b); const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        int nx=bk.getNx(),ny=bk.getNy(),nz=bk.getNz(),ov=bk.getOverlap();
    ...
    // wall phi ghost fix (Z direction)
    {
      auto& pF=PFLattice.getField<PHI<T>>();
      for(int b=0;b<Geo.getBlockNum();++b){
        const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
    ...
    // wall grad/chempot fix (PF) — Z direction only, 3D extended with jj loop
    {
      ...
      for(int b=0;b<Geo.getBlockNum();++b){
        ...
        T mz=bk.getMin()[2],Mz=bk.getMax()[2];
        if(mz<Cell_Len*T{1.5}) for(int jj=0;jj<ny;++jj) for(int i=ov;i<nx-ov;++i) bG.get(ov*pr[2]+jj*pr[1]+i)[2]=bG.get((ov+1)*pr[2]+jj*pr[1]+i)[2];
        if(Mz>H_global-Cell_Len*T{1.5}){int kk=bk.getNz()-1-ov;for(int jj=0;jj<ny;++jj)for(int i=ov;i<nx-ov;++i)bG.get(kk*pr[2]+jj*pr[1]+i)[2]=bG.get((kk-1)*pr[2]+jj*pr[1]+i)[2];}
      }
    }
    {
      ...
      for(int b=0;b<Geo.getBlockNum();++b){
        ...
        T mz=bk.getMin()[2],Mz=bk.getMax()[2];
        if(mz<Cell_Len*T{1.5}) for(int jj=0;jj<ny;++jj) for(int i=ov;i<nx-ov;++i) bC.get(ov*pr[2]+jj*pr[1]+i)=(T{4}*bC.get((ov+1)*pr[2]+jj*pr[1]+i)-bC.get((ov+2)*pr[2]+jj*pr[1]+i))/T{3};
        if(Mz>H_global-Cell_Len*T{1.5}){int kk=bk.getNz()-1-ov;for(int jj=0;jj<ny;++jj)for(int i=ov;i<nx-ov;++i)bC.get(kk*pr[2]+jj*pr[1]+i)=(T{4}*bC.get((kk-1)*pr[2]+jj*pr[1]+i)-bC.get((kk-2)*pr[2]+jj*pr[1]+i))/T{3};}
      }
    }
    ff::CommunicateAllSelfFields<T>(PFLattice);
  ...
  Printer::Print("Total PhysTime", BaseConv.getPhysTime(t()));
  Printer::Endl();
  return 0;
}
