...
int main(int argc, char* argv[]) {
  ...
  printf("Initializing Bubble Rising in Ferrofluid (3D)...\n");
  readParam();
  ...
  // -- init phi (spherical tanh bubble) --
  ...
  while(t()<MaxStep){
    ...
    // 0b: Set wall psi = -H0*z and wall pops = feq(psi_bc)  (top & bottom z-walls)
    {
      auto& psiF=MFLattice.getField<PSI<T>>();
      for(int b=0;b<Geo.getBlockNum();++b){
        auto& bl=MFLattice.getBlockLat(b); const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        ...
        T mz=bk.getMin()[2],Mz=bk.getMax()[2],vs=bk.getVoxelSize();
        if(mz<Cell_Len*T{1.5}){
          for(int kk=0;kk<=ov;++kk){
            T z=mz+T(kk)*vs, psi_w=-H0*z;
            for(int j=0;j<ny;++j)
              for(int i=0;i<nx;++i){
                std::size_t id=kk*pr[2]+j*pr[1]+i; MFCELL c(id,bl);
                for(unsigned q=0;q<MFLatSet::q;++q) c[q]=latset::w<MFLatSet>(q)*psi_w;
                bPsi.get(id)=psi_w;
              }
          }
        }
        if(Mz>H_global-Cell_Len*T{1.5}){
    ...
    // ===== Phase D: Streaming =====
    PF_BB.Apply(t());
    { // PF z ghost pop fix (top & bottom walls)
      T Hg=T(Nk)*Cell_Len;
      for(int b=0;b<Geo.getBlockNum();++b){
        ...
        if(mz<Cell_Len*T{1.5})
          for(int j=0;j<ny;++j) for(int i=0;i<nx;++i){
            PFCELL g(0*pr[2]+j*pr[1]+i,bl),w(ov*pr[2]+j*pr[1]+i,bl);
            for(unsigned q=0;q<LatSet::q;++q)g[q]=w[q];
          }
        if(Mz>Hg-Cell_Len*T{1.5})
          for(int j=0;j<ny;++j) for(int i=0;i<nx;++i){
            PFCELL g((nz-1)*pr[2]+j*pr[1]+i,bl),w((nz-1-ov)*pr[2]+j*pr[1]+i,bl);
            for(unsigned q=0;q<LatSet::q;++q)g[q]=w[q];
          }
      }
    }
    ...
    NS_BB.Apply(t());
    { // NS z ghost pop fix (top & bottom walls)
      T Hg=T(Nk)*Cell_Len;
      for(int b=0;b<Geo.getBlockNum();++b){
        auto& bl=NSLattice.getBlockLat(b); const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        int nx=bk.getNx(),ny=bk.getNy(),nz=bk.getNz(),ov=bk.getOverlap();
        ...
        if(mz<Cell_Len*T{1.5})
          for(int j=0;j<ny;++j) for(int i=0;i<nx;++i){
            NSCELL g(0*pr[2]+j*pr[1]+i,bl),w(ov*pr[2]+j*pr[1]+i,bl);
            for(unsigned q=0;q<LatSet::q;++q)g[q]=w[q];
          }
        if(Mz>Hg-Cell_Len*T{1.5})
          for(int j=0;j<ny;++j) for(int i=0;i<nx;++i){
            NSCELL g((nz-1)*pr[2]+j*pr[1]+i,bl),w((nz-1-ov)*pr[2]+j*pr[1]+i,bl);
            for(unsigned q=0;q<LatSet::q;++q)g[q]=w[q];
          }
      }
    }
    ...
    // wall grad/chempot fix (PF, z walls)
    {
      auto& gF=PFLattice.getField<GRAD<T,3>>();
      for(int b=0;b<Geo.getBlockNum();++b){
        const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        auto& bG=gF.getBlockField(b); int nx=bk.getNx(),ny=bk.getNy(),nz=bk.getNz(),ov=bk.getOverlap();
        T mz=bk.getMin()[2],Mz=bk.getMax()[2];
        if(mz<Cell_Len*T{1.5}) for(int j=ov;j<ny-ov;++j) for(int i=ov;i<nx-ov;++i) bG.get(ov*pr[2]+j*pr[1]+i)[2]=bG.get((ov+1)*pr[2]+j*pr[1]+i)[2];
        if(Mz>H_global-Cell_Len*T{1.5}){int kk=nz-1-ov;for(int j=ov;j<ny-ov;++j) for(int i=ov;i<nx-ov;++i) bG.get(kk*pr[2]+j*pr[1]+i)[2]=bG.get((kk-1)*pr[2]+j*pr[1]+i)[2];}
      }
    }
    {
      ...
      for(int b=0;b<Geo.getBlockNum();++b){
        const auto& bk=Geo.getBlock(b); const auto& pr=bk.getProjection();
        auto& bC=cF.getBlockField(b); int nx=bk.getNx(),ny=bk.getNy(),nz=bk.getNz(),ov=bk.getOverlap();
        T mz=bk.getMin()[2],Mz=bk.getMax()[2];
        ...
        if(Mz>H_global-Cell_Len*T{1.5}){int kk=nz-1-ov;for(int j=ov;j<ny-ov;++j) for(int i=ov;i<nx-ov;++i) bC.get(kk*pr[2]+j*pr[1]+i)=(T{4}*bC.get((kk-1)*pr[2]+j*pr[1]+i)-bC.get((kk-2)*pr[2]+j*pr[1]+i))/T{3};}
      }
    }
    ff::CommunicateAllSelfFields<T>(PFLattice);
