#!/usr/bin/env python3
"""analyze_rt.py — 3D Rayleigh-Taylor interface analysis from rtMag3d vti output

Usage: python3 analyze_rt.py <workdir> [z0] [--png dir]

Reads PHI from block-structured 3D VTI files of one output step, assembles
the global domain, computes the interface height map h(x,y) (phi=0.5
crossing, heavy ferrofluid on TOP / phi=1 above) and reports:
  - light_front = max h  (light bubble rises at trough -> interface highest)
  - heavy_tip    = min h  (heavy spike descends at peak -> interface lowest)
  - amp = (hmax - hmin)/2
  - bulk |H| uniformity check (mean |H| in a light-fluid bulk slab)
"""
...
def load_step(step, vtkdir):
    files = sorted(glob.glob(f"{vtkdir}/vtidata/rtMag3d_T{step}_B*.vti"))
    if not files:
        return None
...
def interface_height(phi, z0_guess):
    """Heavy ferrofluid on TOP (phi=1 above interface). The interface is the
    upward phi=0.5 crossing: first cell (from z=0) where phi >= 0.5."""
    nz = phi.shape[2]
    above = phi >= 0.5
...
def main():
    ...
    ap.add_argument("z0", type=float, nargs="?", default=256.0, help="initial interface z")
    ...
    steps = sorted(set(int(re.search(r"_T(\d+)_", p).group(1))
                       for p in glob.glob(f"{vtkdir}/vtidata/rtMag3d_T*_B0.vti")))
    if not steps:
    ...
    for s in steps:
        ...
        light_front = h.max()   # 轻流体前锋 (最高界面, 轻气泡上浮)
        heavy_tip = h.min()     # 重流体尖钉 (最低界面, 重尖钉下探)
        amp = 0.5 * (light_front - heavy_tip)
        bulk_hmag = d["hmag"][:, :, 5:15].mean()
        print(f"t={s:5d}  light_front={light_front:8.3f} heavy_tip={heavy_tip:8.3f} "
              f"amp={amp:7.3f}  bulk|H|={bulk_hmag:.4f}")
        if args.png:
            try:
                import matplotlib
                matplotlib.use("Agg")
                ...
                im = ax.imshow(h, origin="lower", cmap="RdBu_r", vmin=h.min(), vmax=h.max())
                plt.colorbar(im, ax=ax, label="interface z")
                ax.set_title(f"t={s}  front={light_front:.2f} tip={heavy_tip:.2f}")
                ax.set_xlabel("x"); ax.set_ylabel("y")
                fig.savefig(os.path.join(args.png, f"rt_h_t{s:05d}.png"), dpi=100)
                plt.close(fig)
