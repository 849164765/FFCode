#!/usr/bin/env python3
"""analyze_kh.py — 3D Kelvin-Helmholtz interface analysis from khMag3d vti output

Usage: python3 analyze_kh.py <workdir> [z0] [--png dir]
...
(phi=0.5 crossing, ferrofluid on BOTTOM / phi=1 below) and reports:
  - mixed_layer_h = hmax - hmin  (interface vertical spread as it rolls up)
  - hmax / hmin
  - bulk |H| uniformity check (should be ~H0 in both fluids for horizontal field)
"""
...
def load_step(step, vtkdir):
    files = sorted(glob.glob(f"{vtkdir}/vtidata/khMag3d_T{step}_B*.vti"))
    if not files:
        return None
...
def interface_height(phi, z0_guess):
    """Ferrofluid on BOTTOM (phi=1 below, solvent on top). Interface = the
    upward phi=0.5 crossing: first cell (from z=0) where phi < 0.5."""
    nz = phi.shape[2]
    above = phi >= 0.5
    cross = np.argmax(~above, axis=2)   # shape (nx, ny): z-index of first light cell
    h = np.zeros(phi.shape[:2])         # shape (nx, ny)
    ...
    for jy in range(ny):
        for jx in range(nx):
            ...
            if k == 0 or k >= nz - 1:
                h[jx, jy] = z0_guess
                continue
            p0, p1 = phi[jx, jy, k - 1], phi[jx, jy, k]
            if p1 - p0 == 0:
                h[jx, jy] = k
            else:
                h[jx, jy] = (k - 1) + (0.5 - p0) / (p1 - p0)
    return h
...
def main():
    ...
    ap.add_argument("z0", type=float, nargs="?", default=128.0, help="initial interface z")
    ...
    steps = sorted(set(int(re.search(r"_T(\d+)_", p).group(1))
                       for p in glob.glob(f"{vtkdir}/vtidata/khMag3d_T*_B0.vti")))
    if not steps:
    ...
    for s in steps:
        ...
        mixed = h.max() - h.min()   # 混合层高度 (界面卷曲使高度展宽)
        bulk_hmag = d["hmag"][:, :, 8:28].mean()
        print(f"t={s:5d}  mixed_layer_h={mixed:8.3f}  (hmax={h.max():8.3f} hmin={h.min():8.3f})"
              f"  bulk|H|={bulk_hmag:.4f}")
        if args.png:
            try:
                import matplotlib
                matplotlib.use("Agg")
                ...
                ax.set_title(f"t={s}  mixed_layer_h={mixed:.2f}")
                ax.set_xlabel("x"); ax.set_ylabel("y")
                fig.savefig(os.path.join(args.png, f"kh_h_t{s:05d}.png"), dpi=100)
                plt.close(fig)
