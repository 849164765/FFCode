// rti2d.cpp
// 2D Rayleigh-Taylor Instability simulation - Phase field + NS flow coupling
...
void readParam() {
  ...
  MPI_RANK(0) {
    ...
    std::cout << "===== RTI 2D Simulation (MRT) =====\n"
...
int main(int argc, char* argv[]) {
  ...
  Printer::Print_BigBanner("Initializing RTI 2D (MRT)...");
  readParam();
  ...
  vtmo::vtmWriter<T, 2> GeoWriter("GeoFlag_RTIMRT", Geo, 1);
  ...
  vtmo::vtmWriter<T, 2> MainWriter("rti2dMRT", Geo);
  ...
  Printer::Print_BigBanner("Starting RTI 2D MRT Calculation...");
  ...
  while (MainLoopTimer() < MaxStep) {
    ...
    // Step 2: Compute grad-phi, laplacian, chemical potential mu
    ...
    // Step 3: Compute grad-mu -> NORMAL (Cahn-Hilliard source direction)
  ...
  Printer::Print_BigBanner("RTI 2D MRT Calculation Complete!");
  MainWriter.WriteBinary(MainLoopTimer());
