...
void readParam() {
  iniReader param_reader("bubble_paper2d.ini");
...
int main(int argc, char* argv[]) {
  ...
  // ------------------ define NS lattice ------------------
  // RHO<T> stores p* (pressure) in the g-LBE formulation (Eq.32a)
  // VELOCITY stores u from the g-populations (Eq.32b)
  // Physical density rho(phi) enters through the force computation (FFAllForces2D)
  ...
  // Initialize NS g-populations to pressure-based g-LBE equilibrium (u=0)
  // g_eq_alpha = p* * w_alpha + (Gamma_alpha - w_alpha) = cs2 * w_alpha  (u=0)
  T p_star_init = LatSet::cs2;
  for (int blockid = 0; blockid < Geo.getBlockNum(); ++blockid) {
    const auto& block = Geo.getBlock(blockid);
    const auto& proj = block.getProjection();
    ...
    for (int j = overlap; j < block.getNy() - overlap; ++j) {
      for (int i = overlap; i < block.getNx() - overlap; ++i) {
        ...
        for (unsigned int k = 0; k < LatSet::q; ++k) {
          cell[k] = p_star_init * latset::w<LatSet>(k);
        }
      }
    }
  }
  ...
  // ------------------ define tasks ------------------
  // NS tasks: pressure-based g-LBE (Fakhari et al. 2017)
  // Task A: Update macroscopic fields (p* and u) from g-populations
  using UpdatePVGTask = tmp::Key_TypePair<
    BulkFlag, phase_field::UpdatePressureVelocityG<NSCELL>>;
  using UpdatePVGTaskSelector = TaskSelector<std::uint8_t, NSCELL, UpdatePVGTask>;

  // Task B: g-LBE BGK collision (Eqs. 14-17)
  using GColTask = tmp::Key_TypePair<
    BulkFlag, phase_field::G_Collision<NSCELL>>;
  using GColTaskSelector = TaskSelector<std::uint8_t, NSCELL, GColTask>;
  ...
  // ---- Coupling task (PF → NS): All forces ----
  ...
  using AllForcesTask =
    tmp::Key_TypePair<BulkFlag, ff::FFAllForces2D<PFCELL, NSCELL>>;
  using AllForcesTaskSelector =
    CoupledTaskSelector<std::uint8_t, PFCELL, NSCELL, AllForcesTask>;
  BlockLatManagerCoupling AllForcesCoupling(PFLattice, NSLattice);
  ...
  while (MainLoopTimer() < MaxStep) {
    ...
    // ---- NS force accumulation (pressure-based g-LBE) ----
    // Step 3: Clear NS FORCE
    NSLattice.getField<FORCE<T, LatSet::d>>().InitValue(Vector<T, 2>{T{0}, T{0}});

    // Step 4: Compute ALL forces F = F_s + F_p + F_b (Eq.18)
    // F_p = -p* * cs^2 * grad(rho) is the critical missing piece
    // p* is computed from NS g-populations inside FFAllForces2D
    AllForcesCoupling.ApplyCellDynamics<AllForcesTaskSelector>(MainLoopTimer(), FlagFM);

    // ---- NS collision and streaming (g-LBE) ----
    // Step 5: Update macroscopic fields from g-populations
    // p* = sum(g), u = sum(g*e) + F/(2*p*)  → writes RHO, VELOCITY
    NSLattice.template ApplyCellDynamics<UpdatePVGTaskSelector>(FlagFM);

    // Step 6: g-LBE BGK collision (Eqs. 14-17), reads RHO, VELOCITY, FORCE
    NSLattice.template ApplyCellDynamics<GColTaskSelector>(FlagFM);
