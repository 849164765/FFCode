...
std::vector<std::vector<T>>& SendBuffers, const std::vector<DistributedComm>& MPISends) {
  ...
  for (std::size_t i = 0; i < MPISends.size(); ++i) {
    ...
    const std::size_t size = sends.size();
    std::size_t bufidx{};
    for (std::size_t iArr = 0; iArr < size;) {
      if constexpr (LatSet::d == 2) {
        popIntp<0>(OmegaC, sends, RhoArr, UArr, iArr            , buffer, bufidx); 
        popIntp<1>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<2>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<3>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        iArr += SendSize;
      } else if constexpr (LatSet::d == 3) {
        popIntp<0>(OmegaC, sends, RhoArr, UArr, iArr            , buffer, bufidx); 
        popIntp<1>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<2>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<3>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<4>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<5>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<6>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        popIntp<7>(OmegaC, sends, RhoArr, UArr, iArr += SendSize, buffer, bufidx); 
        iArr += SendSize;
      }
    }
    // non-blocking send
